...
class MultiClassDataLoader(object):
    ...
    def define_flags(self):
        self.__flags.DEFINE_string("train_data_file", "./train_eng1.kkk", "Data source for the training data.")
        self.__flags.DEFINE_string("dev_data_file", "./train_eng2.kkk", "Data source for the cross validation data.")
    ...
    def load_predict_data(self):
        x_eval = self.__load_data("./train_eng2.kkk")
        return x_eval
    ...
    def __load_data(self, data_file):
        ...
        with open(data_file, 'r') as tsvin:
            for row in tsvin:
                data = self.__data_processor.clean_data(row)
                x_text.append(data)
        return x_text
